...
for i in range(len(df)):
    ...
    if orien == '-':
            ...
            posit_new = int(posit) + 1 #why shift to the right by 1? posit_new = the 5' end of the ribosome-covered fragment
    counts.append(1)
...
names = []
chroms = []
posits = [] # this is the start (5' end) of the ribosome-covered fragmemnt.
rposits = []
counts = []
starts = []
ends = []
...
for chromfile in chromfiles_s:
    ...
    for i in range(len(df_chrom)):
        ...
        posit_new = df_chrom.iloc[i, -2] #The posit_new here is the 5' ened start of the ribosome-covered fragment.
        count = df_chrom.iloc[i, -1]
        #print(orien,posit_new,count)
        if orien == '-':             # for - strand, the end posit of the gene should be greater than start posit of the gene fragment -16 && the start posit of the gene should be less than the start posit of fragment -13
